...
row_template = """
  ...
  """
html_template = """
  <script type="application/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
  <script type="application/javascript">
    $(document).ready(function(){
      $('.diff_play').on('mouseover', function(){
        $cur_id = $(this).children("span").prop("id");
    ...
    for result in os.listdir(result_dir):
        try:
            ...
            if result.endswith(suffix):
                ...
                id_check = defaultdict(int)
                same_target_token = defaultdict(int)
                        ...
                        if n1 in func_features1[func1] and n2 in func_features2[func2]:
                            ...
                            for i in range(0, len(toklst1), 2):
                                if i + 1 < len(toklst1):
                                    ...
                                    temp_id = id
                                    if n2 in same_target_token:
                                        temp_id = same_target_token[n2]
                                    elif line_num in id_check:
                                        temp_id = id_check[line_num]
                                    else:
                                        id_check[line_num] = id
                                    matched_token_html1[toklst1[i]] = [f'@@id="token_{temp_id}"##',len(toklst1[i+1]),toklst1[i+1]]
                                    #print(matched_token_html1[toklst1[i]])
                                    for ind in range(int(col_num), int(col_num)+length):
                                        matchset1[int(line_num)][ind]=1                     
                            same_target_token[n2] = temp_id
                            toklst2 = nodefeatures2[n2][-2].split('@*@')
                            ...
                            for i in range(0, len(toklst2), 2):
                                if i + 1 < len(toklst2):
                                    line_num, col_num = toklst2[i].split(':')
                                    length = len(toklst2[i+1])
                                    matched_token_html2[toklst2[i]] = [f'@@id="token_{temp_id}"##',len(toklst2[i+1])]
                                    for ind in range(int(col_num), int(col_num)+length):
